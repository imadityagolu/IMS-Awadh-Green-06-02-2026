...
function CreateShoppingPoints() {
  ...
  const [selected, setSelected] = useState("Shopping Points");
  ...
  const handleSetupNext = async () => {
    ...
    if (!amountForPoint || amountForPoint <= 0) {
      toast.error("Valid amount for 1 point is required in Reward Setup");
      switchTab("reward");
      return;
    }
    if (!minPurchase || minPurchase <= 0) {
      toast.error("Valid minimum purchase amount is required in Reward Setup");
      switchTab("reward");
      return;
    }
    if (!deadline) {
      toast.error("Deadline is required in Reward Setup");
      switchTab("reward");
      return;
    }
    ...
    if (!pointValue || pointValue <= 0) {
      toast.error("Valid point value is required in Redeem Setup");
      switchTab("redeem");
      return;
    }
    if (!maxEligibleAmount || maxEligibleAmount <= 0) {
      toast.error("Valid max eligible amount is required in Redeem Setup");
      switchTab("redeem");
      return;
    }
    if (!minInvoiceValue || minInvoiceValue <= 0) {
      toast.error("Valid minimum invoice value is required in Redeem Setup");
      switchTab("redeem");
      return;
    }
    ...
    try {
      ...
      setCurrentStep(3);
      toast.success("Reward system created successfully");
    } catch (err) {
