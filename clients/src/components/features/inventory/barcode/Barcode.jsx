...
function Barcode() {
  ...
  // Re-render barcodes when modal opens or product data changes
  useEffect(() => {
      ...
      const timer = setTimeout(() => {
        ...
        for (let i = 0; i < count; i++) {
          ...
          if (element) {
            try {
              let format = 'CODE128';
              // if (/^\d{12,13}$/.test(code)) format = 'EAN13';
  ...
  const generateBarcode = () => {
    ...
    (async () => {
      try {
        ...
        // Barcode rendering is now handled by the useEffect
      } catch (err) {
    ...
    <div className="p-4">
      ...
      <div style={{ overflow: "auto", height: "calc(100vh - 150px)" }}>
        <div className='card p-4'>
                          ...
                          <div>
                            <div style={{ fontWeight: '500', color: '#333', display: 'flex', alignItems: 'center', gap: '10px' }}>
                              {productItem?.images[0]?.url ? (
                                ...
                                >
                                  {productItem.productName?.charAt(0).toUpperCase() || "U"}
                                </div>
                              )}
          ...
          {!selectedProduct && (
            <div style={{ border: '1px solid #e6e6e6ff', color: "#999797ff", backgroundColor: "white", padding: '40px', borderRadius: '8px', marginTop: '24px', textAlign: 'center' }}>
