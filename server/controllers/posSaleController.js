...
const createPosSale = async (req, res) => {
  try {
    ...
    try {
      ...
      try {
        for (const item of items) {
          const product = await Product.findById(item.productId).select('stockQuantity');
          if (product) {
            const currentOpening = Number(product.stockQuantity || 0);
            const qty = Number(item.quantity || 0);
            const newOpening = Math.max(0, currentOpening - qty);
            await Product.findByIdAndUpdate(item.productId, { $set: { stockQuantity: newOpening } });
          }
        }
      } catch (openingErr) {}
